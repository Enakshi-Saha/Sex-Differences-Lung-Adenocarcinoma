...
/GDSC_code/paper_LUAD_drugResponse.R	
		
# Figure D.1: Scatterplot of first two principal components of log-transformed raw counts in GTEx (left) and TCGA (right) data.
GTEx_TCGA_batchEffect.R

# Figure D.2: Scatterplot of first two principal components of log-transformed raw counts in GSE68465 before (left) and after (right) batch correction.
GTEx_TCGA_batchEffect.R

# Figure D.3 (top row): Identifying samples with a possible mismatch between gender and biological sex defined based on Y chromosome gene expression in GTEx (top left), TCGA (top right), LGRC (bottom left) and GSE68465 (bottom right)
GTEx_TCGA_processing.R

# Figure D.3 (bottom row): Identifying samples with a possible mismatch between gender and biological sex defined based on Y chromosome gene expression in GTEx (top left), TCGA (top right), LGRC (bottom left) and GSE68465 (bottom right)
GSE68465_processing.R

# Figure D.4: Sex Difference in LGRC control lung samples within nonsmokers and smokers
sexDiff_heatmaps.R

# Figure D.5: Sex Difference in tumor samples from the validation data GSE68465 within nonsmokers and smokers
sexDiff_heatmaps.R

# Figure D.6: Sex Difference in immune and stromal cell composition in GTEx samples: nonsmokers (left) and smokers (right)
